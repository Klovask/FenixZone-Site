<?php  require_once "seguridad/sql_inject.php"; $bDestroy_session = TRUE; $url_redirect = 'index.php'; $sqlinject = new sql_inject('./log_file_sql.log',$bDestroy_session,$url_redirect); session_start(); error_reporting(0); include_once('./kev/pdo.php'); if(isset($_SESSION['User'])) { $User = $_SESSION['User']; try { $stmt = $con->prepare("SELECT * FROM usuarios WHERE Username = :usuario"); $stmt->bindParam(':usuario', $User, PDO::PARAM_STR); $stmt->execute(); while($datos = $stmt->fetch()) { $name = $datos['Username']; $password = $datos['Password']; $money = $datos['Money']; $score = $datos['Nivel']; $fz = $datos['Moneda']; $ropa = $datos['Skin']; $platabanco = $datos['Banco']; $faccion = $datos['Faccion']; $razon = $datos['razon']; $ban = $datos['Baneado']; $Conexion = $datos['Conexion']; } } catch(PDOException $e) { echo 'Error: ' . $e->getMessage(); } } else echo "<script>window.location='ingresar.php';</script>"; $dinerototal = $money+$platabanco; if($Conexion == '1') { echo "<script>window.location='reg.php?u=2';</script>"; } ?>
...
<?php function validarCadena($cadena) { $permitidos = "abcdefghijklmnopqrstuvwxyzñABCDEFGHIJKLMNOPQRSTUVWXYZÑ0123456789"; for ($i=0; $i<strlen($cadena); $i++) { if (strpos($permitidos, substr($cadena,$i,1))===false) { return false; } } return true; } ?>
...
<!--<?php  $submit = $_POST['commit']; $contraactual = $_POST['vieja']; $contranueva = $_POST['nueva']; $contranueva2 = $_POST['nueva2']; if($submit) { if($estaonline == 0) { if($contraactual || $contranueva || $contranueva2) { if($contraactual == $password) { if($contranueva == $contranueva2) { if(validarCadena($contraactual) == true && validarCadena($contranueva) == true && validarCadena($contranueva2) == true) { $stmt = $con->prepare("UPDATE usuarios SET Password = :password WHERE Username = :usuario"); $stmt->bindParam(':password', $contranueva, PDO::PARAM_STR); $stmt->bindParam(':usuario', $name, PDO::PARAM_STR); $stmt->execute(); $stm2 = $con->prepare("UPDATE smf_members SET passwd=SHA1(CONCAT(LOWER(:usuario), :password)) WHERE member_name = :usuario"); $stm2->bindParam(':password', $contranueva, PDO::PARAM_STR); $stm2->bindParam(':usuario', $name, PDO::PARAM_STR); $stm2->execute(); echo '<div class="login" style="padding:5px;top:20px;background-color:#06AD00; color:#FFFFFF; width:560px; text-align:center;"><strong>Cambio realizado correctamente</strong></div>'; session_destroy(); } else { echo '<div class="login" style="padding:5px;top:20px;background-color:#F00; color:#FFFFFF; width:560px; text-align:center;"><strong>No puedes usar caracteres invalidos en la contrase&ntilde;a</strong></div>'; } } else { echo '<div class="login" style="padding:5px;top:20px;background-color:#F00; color:#FFFFFF; width:560px; text-align:center;"><strong>Las contrase&ntilde;as no son iguales</strong></div>'; } } else { echo '<div class="login" style="padding:5px;top:20px;background-color:#F00; color:#FFFFFF; width:560px; text-align:center;"><strong>La contrase&ntilde;a actual ingresada no es correcta</strong></div>'; } } else { echo '<div class="login" style="padding:5px;top:20px;background-color:#F00; color:#FFFFFF; width:560px; text-align:center;"><strong>Te faltaron ingresar los datos.</strong></div>'; } } else { echo '<div class="login" style="padding:5px;top:20px;background-color:#F00; color:#FFFFFF; width:667px; text-align:center;"><strong>Tienes que estar desconectado para cambiarte la contrase&ntilde;a.</strong></div>'; } } else { echo '<div class="login" style="padding:5px;top:20px;background-color:#F00; color:#FFFFFF; width:560px; text-align:center;"><strong>IMPORTANTE: Si tienes otra cuenta en ';?><?php echo $NombreServidor?><?php echo ', elige una contrase&ntilde;a diferente para aumentar la seguridad de tus cuentas.</strong></div>'; } ?>-->
...
<input name="viejaa" type="password" maxlength="30" placeholder="Contrase&ntilde;a" style="display:none" autocomplete="off">
<input name="cambiar" type="hidden" value="1" >
<p><input name="vieja" type="password" maxlength="30" placeholder="Contrase&ntilde;a" autocomplete="off"></p>
<p><input name="nueva" type="password" maxlength="30" placeholder="Nueva contrase&ntilde;a" autocomplete="off"></p>
<p><input name="nueva2" type="password" maxlength="30" placeholder="Repetir nueva contrase&ntilde;a" autocomplete="off"></p>
<p><input type="submit" name="commit" value="Cambiar contrase&ntilde;a"></p>
...
<a href="cc.php" class="actual1" title="Cambia tu contrase&ntilde;a">&raquo; Cambiar contrase&ntilde;a</a>
