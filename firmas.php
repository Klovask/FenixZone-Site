...
<?php  require_once "seguridad/sql_inject.php"; $bDestroy_session = TRUE; $url_redirect = 'index.php'; $sqlinject = new sql_inject('./log_file_sql.log',$bDestroy_session,$url_redirect); if(isset($_SESSION['User'])) { $User = $_SESSION['User']; $stmt = $con->prepare("SELECT * FROM usuarios WHERE Username = :usuario"); $stmt->bindParam(':usuario', $User, PDO::PARAM_STR); $stmt->execute(); $num_rows = $stmt->rowCount(); if($num_rows > 0) { while($row = $stmt->fetch()) { echo'
			<img src="./imagenes/firmas/firma1.png'.$_SERVER['HTTP_HOST'].''.$NombreCarpeta.'/firma.php?t=1&n='.$row['ID'].'"/>
			<br/>
			...
			<hr/>
			<img src="./imagenes/firmas/firma2.png'.$_SERVER['HTTP_HOST'].''.$NombreCarpeta.'/firma.php?t=2&n='.$row['ID'].'"/>
			<br/>
			...
			<hr/>
			<img src="./imagenes/firmas/firma3.png'.$_SERVER['HTTP_HOST'].''.$NombreCarpeta.'/firma.php?t=3&n='.$row['ID'].'"/>
			<br/>
			...
			<hr/>
			<img src="./imagenes/firmas/firma4.png'.$_SERVER['HTTP_HOST'].''.$NombreCarpeta.'/firma.php?t=4&n='.$row['ID'].'"/>
			<br/>
...
<hr/>
			<img src="./imagenes/firmas/firma5.png'.$_SERVER['HTTP_HOST'].''.$NombreCarpeta.'/firma.php?t=5&n='.$row['ID'].'"/>
			<br/>
...
<hr/>
			<img src="./imagenes/firmas/firma6.png'.$_SERVER['HTTP_HOST'].''.$NombreCarpeta.'/firma.php?t=6&n='.$row['ID'].'"/>
			<br/>
...
<hr/>
			<img src="./imagenes/firmas/firma7.png'.$_SERVER['HTTP_HOST'].''.$NombreCarpeta.'/firma.php?t=7&n='.$row['ID'].'"/>
			<br/>
			<textarea name="" cols="" rows="" style="width:600px; height:30px; font-size:12px;">[url=./imagenes/firmas/firma7.png'.$_SERVER['HTTP_HOST'].'/nuevo.php?u='.$row['ID'].'][img]./imagenes/firmas/firma7.png'.$_SERVER['HTTP_HOST'].'/'.$NombreCarpeta.'firma.php?t=7&n='.$row['ID'].'[/img][/url]</textarea>
